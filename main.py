...
def bfs_search():
    ...
    while (len(queue) != 0):
        ...
        if current_state.l_child != -1:
            ...
            dot.node(str(child_state.self_id), child_state.__str__())
            dot.edge(str(current_state.self_id), str(
                child_state.self_id), label="left")
            if child_state.self_state == final_state:
                break
        if current_state.d_child != -1:
            ...
            dot.node(str(child_state.self_id), child_state.__str__())
            dot.edge(str(current_state.self_id), str(
                child_state.self_id), label="down")
            if child_state.self_state == final_state:
                break
        if current_state.r_child != -1:
            ...
            dot.node(str(child_state.self_id), child_state.__str__())
            dot.edge(str(current_state.self_id), str(
                child_state.self_id), label="right")
            if child_state.self_state == final_state:
                break
...
def dfs(node):
    ...
    if node not in visited:
        ...
        dot_dfs.edge(str(node.parent_id), str(
            node.self_id), label=node.parent_move)
