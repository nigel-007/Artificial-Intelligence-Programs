    ...
    while True:
        ...
        if current_state.u_child != -1:
            ...
            if child_state not in explored and child_state not in unexplored:
                ...
                dot.edge(str(current_state.self_id), str(
                    child_state.self_id), label=child_state.parent_move)
    ...
    while True:
        ...
        if current_state.u_child != -1:
            ...
            if child_state not in explored and child_state not in unexplored:
                ...
                # graph
                dot.node(str(child_state.self_id), child_state.__str__())
                dot.edge(str(current_state.self_id), str(child_state.self_id),
                         label=child_state.parent_move+str(child_state.cost))
        if current_state.d_child != -1:
            child_state = id_to_state[current_state.d_child]
            cost_child = manhattan_distance(child_state.self_state)
            child_state.cost = cost_child
...
if __name__ == "__main__":
    ...
    opposite_movements = {"up": "down", "down": "up",
    ...
    while (True):
        ...
        for move in possible_movements:
            ...
            if matrix != -1:
                state_obj = State(matrix, current_state.self_id,
                                  move, current_state.level+1)
                ...
                if matrix == final_state:
                    dot.node(str(state_obj.self_id),
                             state_obj.__str__(), color='green')
                else:
                    dot.node(str(state_obj.self_id), state_obj.__str__())

                dot.edge(str(state_obj.parent_id), str(
                    state_obj.self_id), label=move)

    A_star()
    Greedy_best_first()
    dot.render(
        str(os.getcwd() + '/outputs/state_space_tree_informed.gv'), view=True)
